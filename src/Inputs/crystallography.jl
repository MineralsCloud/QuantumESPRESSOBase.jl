...
function Lattice(::FaceCenteredOrthorhombic, p)
    ...
    return Lattice([[a, 0, c], [a, b, 0], [0, b, c]] / 2)
end
function Lattice(::BodyCenteredOrthorhombic, p)
    a, b, c = p[1] .* (1, p[2], p[3])
    return Lattice([[a, b, c], [-a, b, c], [-a, -b, c]] / 2)
end
...
function Lattice(::CCenteredMonoclinic, p)
    a, r1, r2, cosγ = p[1:4]
    return Lattice(
        a * [[1 / 2, 0, -r2 / 2], [r1 * cosγ, r1 * sin(acos(cosγ)), 0], [1 / 2, 0, r2 / 2]],
    )
end
function Lattice(::BCenteredMonoclinic, p)
    a, r1, r2, _, cosβ = p[1:3]
    return Lattice(
        a * [[1 / 2, r1 / 2, 0], [-1 / 2, r1 / 2, 0], [r2 * cosβ, 0, r2 * sin(acos(cosβ))]],
    )
end
