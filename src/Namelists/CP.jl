...
@with_kw struct ElectronsNamelist <: Namelist
    ...
    @assert(orthogonalization ∈ ("ortho", "Gram-Schmidt"), "Invalid `orthogonalization` $(orthogonalization)!")
    @assert(ortho_eps > 0.0, "`ortho_eps` $(ortho_eps) less or equal 0!")
    @assert(ortho_max >= 1.0, "`ortho_max` $(ortho_max) less than 1!")
    @assert(electron_velocities ∈ ("zero", "default", "change_step") "Invalid `electron_velocities` $(electron_velocities)!")
    @assert(electron_temperature ∈ ("nose", "rescaling", "not_controlled"), "Invalid `electron_temperature` $(electron_temperature)!")
...
@with_kw struct IonsNamelist <: Namelist
    ...
    @assert(ion_dynamics ∈ ("none", "sd", "cg", "damp", "verlet"), "Invalid `ion_dynamics` $(ion_dynamics)!")
    @assert(ion_positions ∈ ("default", "from_input"), "Invalid `ion_positions` $(ion_positions)!")
    @assert(ion_velocities ∈ ("default", "change_step", "random", "from_input", "zero"), "Invalid `ion_velocities` $(ion_velocities)!")
    @assert(ion_nstepe > 0.0, "`ion_nstepe` $ion_nstepe out of range!")
    @assert(ion_temperature ∈ ("nose", "rescaling", "not_controlled"), "Invalid `ion_temperature` $(ion_temperature)!")
...
@with_kw struct CellNamelist <: Namelist
    ...
    @assert(cell_parameters ∈ ("default", "from_input"), "Invalid `cell_parameters` $(cell_parameters)!")
    @assert(cell_dynamics ∈ ("none", "sd", "damp-pr", "pr"), "Invalid `cell_dynamics` $(cell_dynamics)!")
    @assert(cell_velocities ∈ ("zero", "default"), "Invalid `cell_velocities` $(cell_velocities)!")
    @assert(wmass >= 0.0 "`wmass` $wmass out of range!")
    @assert(cell_temperature ∈ ("nose", "rescaling", "not_controlled"), "Invalid `cell_temperature` $(cell_temperature)!")
    @assert(cell_dofree ∈ ("all", "x", "y", "z", "xy", "xz", "yz", "xyz", "shape", "2Dxy", "2Dshape", "volume"))
